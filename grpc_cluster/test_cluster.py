...
def create_worker_fullname(proxy_name, worker_name, worker_port):
    fullname = uri_format.format(worker_name, proxy_name, worker_port)
    return fullname
...
class Cluster(object):
    ...
    class Master(object):
            ...
            class MessageHelper:
                ...
                def discardAllTimeoutTasks(self):
                    ...
                    for ID in self._mapping_task:
                        if checkMappingTaskTimeout(ID):
                            task = discardMappingTask(ID)
                ...
                def getSmallestTimeout(self):
                    _timeout = None
                    for ID in self._mapping_task:
                        timeout = self._mapping_task[ID]['start_time'] + self._mappinf_task[ID]['timeout']
                        if _timeout == None:
                            _timeout = timeout
        ...
        def restart_workers(self, worker_fullnames):
            ...
            for fullname in worker_fullnames:
                ...
                self._add_worker_client(proxy_name, proxy_ip, name, port, config)
        ...
        def _wait_for_receiving_welcome_message(self, worker_list=None):
            ...
            if worker_list == None:
                worker_list = self._newly_added_workers
        ...
        def poll_worker(self, fullname):
            ...
            worker.alive = worker.client.welcome()
            if worker.alive:
        ...
        def poll_proxy(self, proxy_name):
            ...
            proxy.alive = proxy.client.welcome()
        ...
        def reduce(self, block=True):
            ...
            while self.helper.getMappingTaskNum() > 0:
                ...
                if d == None:
                    ...
                    # handle timeout task
                    tasks = self.helper.discardAllTimeoutTasks()
                    
                    # restart timeout workers
                    timeout_workers = []
                    for task in tasks:
                        timeout_workers.append(task['worker_name'])
                        
                    timeout_workers = set(timeout_workers)
                ...
                if token in self._worker_tokens:
                    ...
                    self.helper.moveTaskFromMappingToBuffer(ID, tag, data)
                    
                    worker = self.get_worker(worker_fullname)
                    worker.alive = True
                    worker.working = False
                    
                    worker.tasks.remove(ID)
        ...
        def close(self):
            ...
            for proxy_name in self._worker_client:
                for worker_name in self._worker_client[proxy_name]:
                    ...
                    del self._worker_client[proxy_name][worker_name]
    ...
    def _create_worker_client(self, proxy_config, proxy_ip, worker_name, worker_port, worker_full_config)
    ...
    def _create_cluster(self):
        ...
        # master
        self.parse_master_config(self._config.get('master'))
        
        # create master server
        self._create_master_server(self.master_name, self._master_port, self._master_config, self.max_workers)
            ...
            if proxy_config.get('workers') != None:
                for worker_config in proxy_config.get('workers'):
                    ...
                    def _launch_single_worker_client(worker_name, worker_port):
                        ...
                        self._create_worker_client(proxy_config, proxy_ip, worker_name, worker_port, worker_full_config)
    ...
    def map(self, data_list, tag_list = None):
        self.LOG.debug('call map:')
        
        return self._master_server.map(data_list, tag_list=tag_list)
